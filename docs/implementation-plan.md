...
The existing operating system already owns startup and the blinking-dot screen.
Start Annelie OS directly on home when Chrome kiosk opens. Do not build or
install a second splash or welcome step. Preserve the configured OS startup.
Offer the seven backgrounds described in `docs/design/startup.md`; persist the
